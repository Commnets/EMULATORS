...
- Define methods in the same relative order in which their declarations appear in the class. This applies to constructors, destructors, overloads, out-of-class `inline` definitions in headers, and definitions in `.cpp` files. When adding a method to an existing class, insert its definition at the corresponding position instead of appending it elsewhere. Do not reorder unrelated pre-existing methods unless the requested change includes that cleanup.
...
- Do not introduce an intermediate variable merely to expose a one-use value to an `assert`, to restate an immediately consumed subtraction/comparison, or to name a relationship already guaranteed by the surrounding contract. Use the expression directly and place a concise comment beside it when the invariant is non-obvious. Keep an intermediate only when it prevents repeated work, is reused, is required for safe evaluation order, or materially clarifies genuinely complex logic.
...
- Add `assert` checks only at meaningful contract boundaries or for invariants whose violation would otherwise cause unsafe indexing, invalid ownership, corruption, or an impossible hardware/framework state. Do not add defensive relational `assert` checks inside a trusted internal path when the caller, constructor, type, or immediately preceding selection already guarantees the relation. In particular, do not create temporaries solely to assert that one internally derived value is greater than, less than, or compatible with another; document the relied-upon invariant beside the direct calculation instead.
