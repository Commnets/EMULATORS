...
- Antes de regenerar, determinar el conjunto exacto de emuladores afectados a partir de las clases y productores modificados y de sus consumidores reales. Una clase exclusiva de C64, como VIC-II, solo autoriza actualizar la fuente y el DOCX de C64. Una clase compartida por un subconjunto de máquinas solo afecta a ese subconjunto. Regenerar los seis documentos únicamente cuando cambie una definición, productor, plantilla o infraestructura común que altere realmente el DEBUG de las seis familias.
- Todo cambio que añada, elimine, renombre, reordene o altere una cabecera, campo, evento, unidad, temporización o condición de emisión del fichero DEBUG debe actualizar `.codex/documentation/debug-files/sources/debug-common.yaml` o las fuentes específicas del conjunto afectado y regenerar exclusivamente sus DOCX mediante `.codex/documentation/debug-files/scripts/generate_debug_docs.py --machine <id>`. Repetir `--machine` cuando haya varios afectados; no omitirlo salvo que el conjunto calculado incluya las seis máquinas.
- Limitar al mismo conjunto la actualización de campos, la normalización, la exportación PDF y la revisión visual. No usar un helper masivo si toca documentos ajenos al conjunto afectado; seleccionar los DOCX individualmente o adaptar primero el helper para aceptar esa selección.
- La detección conservadora de fuentes vigiladas y el estado global de sincronización sirven como aviso de revisión, no como autorización para reescribir documentación no afectada. Si el generador o el finalizador exige regeneración global para actualizar su estado, corregir o ampliar esa herramienta antes de continuar; no modificar los otros DOCX como atajo.
...
- Ejecutar `.codex/documentation/debug-files/scripts/check_debug_docs.py` después de regenerar. Su estado es conservador: cualquier cambio en un fichero C++ vigilado obliga a revisar la documentación, aunque finalmente no altere el formato ni amplíe el conjunto de documentos afectados.
