...
- Para comandos, auditar `LocalConsole` y toda la cadena de responsabilidad específica: `C64::CommandBuilder` o `VIC20::CommandBuilder` -> `COMMODORE::CommandBuilder` -> `MCHEmul::StandardCommandBuilder`. Indicar expresamente qué órdenes son exclusivas de la consola local y cuáles admite el canal remoto de la versión sin consola.
