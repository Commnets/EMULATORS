...
namespace FZ80
{
	...
	class Instruction : public MCHEmul::InstructionDefined
	{
		...
		bool IOAccessed () const
							{ return (_IOAccessed); }
		/** Whether the last address-bus access made by the instruction was an I/O access. */
		bool lastINOUTAccessWasIO () const
							{ return (_lastINOUTAccessWasIO); }
		...
		protected:
		/** Calculates and adds the contention generated by an I/O machine cycle.
		...
		private:
		/** Converts an instruction-relative T-state into an absolute CPU T-state. */
		unsigned int instructionClockCycleAt (unsigned int c) const;
		/** Copies the I/O timing data to the outer prefixed instruction, if any. */
		void copyIOExecutionDataToCurrentInstruction (MCHEmul::CPU* c = nullptr);

		private:
		unsigned int _IOAccessClockCycle;
		unsigned int _IOContentionClockCycles;
		bool _IOAccessed;
		bool _lastINOUTAccessWasIO;
	};
	...
	class InstructionUndefined : public MCHEmul::InstructionUndefined
	{
		...
		bool IOAccessed () const
							{ return (_IOAccessed); }
		/** Whether the last access of the selected instruction was an I/O access. */
		bool lastINOUTAccessWasIO () const
							{ return (_lastINOUTAccessWasIO); }
