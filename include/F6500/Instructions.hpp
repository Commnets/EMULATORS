...
namespace F6500
{
	...
	class Instruction : public MCHEmul::InstructionDefined
	{
		...
		/** Writes both values generated by an NMOS 6500 memory RMW instruction:
			the original value first and the modified value afterwards. */
		inline void writeRMWResult (const MCHEmul::Address& a,
			const MCHEmul::UByte& oV, const MCHEmul::UByte& nV);
	};
	...
	// ---
	inline void Instruction::writeRMWResult (const MCHEmul::Address& a,
		const MCHEmul::UByte& oV, const MCHEmul::UByte& nV)
	{
		// NMOS 6500 memory RMW instructions first write back the original value.
		// Both writes are externally visible when the address maps an I/O register.
		memory () -> set (a, oV);
		memory () -> set (a, nV);
	}
	...
	{
		// The original value has to be written back before the shifted one.
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UByte v = oV;
		// Keeps the status of the last bit to actualize later the carry flag
		bool c = v.shiftLeftC (false /** 0 is put into */, 1); 
		writeRMWResult (a, oV, v);
	...
	inline bool DCP_General::executeOn (const MCHEmul::Address& a)
	{
		...
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UInt v = MCHEmul::UInt (oV) - MCHEmul::UInt::_1; // DEC...
		// A carry could be generated, but it will be ignored...
		writeRMWResult (a, oV, v.bytes ()[0]);
	...
	{
		// The original value has to be written back before the decremented one.
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UInt v = MCHEmul::UInt (oV) - MCHEmul::UInt::_1;
		// A carry could be generated, but it will be ignored...
		writeRMWResult (a, oV, v.bytes ()[0]);
	...
	{
		// The original value has to be written back before the incremented one.
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UInt v = MCHEmul::UInt (oV) + MCHEmul::UInt::_1;
		// A carry could be generated, but it will be ignored...
		writeRMWResult (a, oV, v.bytes ()[0]);
	...
	inline bool ROL_General::executeOn (const MCHEmul::Address& a)
	{
		...
		// The original value has to be written back before the rotated one.
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UByte v = oV;
		// Keeps the status of the last bit to actualize later the carry flag
		c = v.rotateLeftC (c /** The carry is put into. */, 1); 
		writeRMWResult (a, oV, v);
	...
	inline bool ROR_General::executeOn (const MCHEmul::Address& a)
	{
		...
		MCHEmul::UByte v = oV;
		// Keeps the status of the last bit to actualize later the carry flag
		c = v.rotateRightC (c /** The carry is put into. */, 1); 
		writeRMWResult (a, oV, v);
	...
	inline bool RRA_General::executeOn (const MCHEmul::Address& a)
	{
		...
		// The operation affects the memory like a ROR operation.
		MCHEmul::UByte oV = memory () -> value (a); // 1 byte long always
		MCHEmul::UByte v = oV;
		// ROR in v, affecting the carry flag...
		st.setBitStatus (C6500::_CARRYFLAG, 
			v.rotateRightC (st.bitStatus (C6500::_CARRYFLAG), 1)); 
		writeRMWResult (a, oV, v);
