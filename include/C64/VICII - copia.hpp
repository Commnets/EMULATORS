...
namespace C64
{
	...
	{
		public:
		/** The sync data describes how th simulation of the vertical and 
			horizontal raster moves in a traditional display. */
		class SyncData
		{
			public:
			SyncData () = delete;
			...
			SyncData (
						unsigned short fsp,		// First Sync position
						unsigned short fvsp,	// First Visible Sync position
						unsigned short fdsp,	// First Display Sync position
						unsigned short ldsp,	// Last Display Sync position
						unsigned short lvsp,	// Last Visible Sync position
						unsigned short lsp,		// Last Sync position
						unsigned short msp		// Maximum Sync positions
					  )
				: _firstSyncPosition (fsp), _firstVisibleSyncPosition (fvsp), _firstDisplaySyncPosition (fdsp),
				  _lastDisplaySyncPosition (ldsp), _lastVisibleSyncPosition (lvsp), _lastSyncPosition (lsp),
				  _maxSyncPositions (msp)
							{ }
			...
			unsigned short toBase0 (unsigned short m) const
							{ return ((m < _firstSyncPosition) 
								? m + _maxSyncPositions - _firstSyncPosition : m - _firstSyncPosition); }

			private:
			unsigned short _firstSyncPosition;
			unsigned short _firstVisibleSyncPosition;
			unsigned short _firstDisplaySyncPosition;
			unsigned short _lastDisplaySyncPosition;
			unsigned short _lastVisibleSyncPosition;
			unsigned short _lastSyncPosition;
			unsigned short _maxSyncPositions;
		};
...
//		VICII (const MCHEmul::Attributes& attrs);
		...
		VICII (const SyncData& vsd, const SyncData& hsd, const MCHEmul::Attributes& attrs);
