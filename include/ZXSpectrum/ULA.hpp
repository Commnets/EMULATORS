...
namespace ZXSPECTRUM
{
	...
	class ULA : public MCHEmul::GraphicalChip
	{
		...
		/** To know whether the active-low /INT line is asserted. */
		bool INTLineActive () const
							{ return (_INTLineActive); }
		...
		void drawEvents ();
		/** Advances the frame counter used to generate the FLASH phase. */
		inline void clockFlashFrame ();
		...
		void requestINTIfNeeded (MCHEmul::CPU* cpu);
		/** To know whether this ULA has already queued an INT request. */
		bool INTRequestPending (const MCHEmul::CPU* cpu) const;
		/** Removes every pending INT request generated by this ULA. */
		void removePendingINTRequest (MCHEmul::CPU* cpu) const;
		...
		bool _INTLineActive;
		/** ULA clocks remaining before releasing the /INT line. */
		unsigned int _INTClocksRemaining;
		/** CPU cycle when the current /INT pulse started. */
		unsigned int _INTStartCPUCycle;
		...
		/** Five-bit frame counter used to derive the FLASH phase. */
		unsigned char _flashFrameCounter;
		/** The number of cycles the CPU was executed once the simulated method finishes. */
		unsigned int _lastCPUCycles;
		...
		struct EventsStatus
		{
			...
			/** The different situations related with NMI and HALT. */
			MCHEmul::OBool _NMIActive, _HALTActivated;
	...
	// ---
	inline void ULA::clockFlashFrame ()
	{
		_flashFrameCounter =
			(_flashFrameCounter + 1) & _FLASHCOUNTERMASK;
	}
	...
	class ULA_PAL final : public ULA
	{
		...
		static double frameFrequency (unsigned int cF)
							{ return (static_cast <double> (cF) /
								static_cast <double> (_CPUTSTATESPERFRAME)); }
