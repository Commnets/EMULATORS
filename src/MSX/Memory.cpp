...
// ---
void MSX::Memory::activeSlotsPerBank (unsigned char sb0, unsigned char sb1, unsigned char sb2, unsigned sb3)
{
	...
	nSS [0] = activeMemoryElementInSlotSubSlotAndBank (sb0, // Slot to be active in the bank 0...
		(MSX::SubSlotRegisters::instance () -> subSlotRegister (sb0).value () & 0x03), 0);
	nSS [1] = activeMemoryElementInSlotSubSlotAndBank (sb1, // Slot to be active in the bank 1...
		(MSX::SubSlotRegisters::instance () -> subSlotRegister (sb1).value () & 0xc0) >> 2, 1);
	nSS [2] = activeMemoryElementInSlotSubSlotAndBank (sb2, // Slot to be active in the bank 2...
		(MSX::SubSlotRegisters::instance () -> subSlotRegister (sb2).value () & 0x30) >> 4, 2);
	nSS [3] = activeMemoryElementInSlotSubSlotAndBank (sb3, // Slot to be active in the bank 3...
		(MSX::SubSlotRegisters::instance () -> subSlotRegister (sb3).value () & 0x03) >> 6, 3);
	...
	if (nStack != nullptr)
	{
		// Reset the system to look for the stack again...
		resetStack ();
		// ..and this should be the id of the new stack...
