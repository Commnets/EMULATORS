...
// ---
C64::VICII::VICII (const C64::VICII::SyncData& vsd, const C64::VICII::SyncData& hsd, 
		const MCHEmul::Attributes& attrs)
	: MCHEmul::GraphicalChip (_ID, attrs),
	  _VSyncData (vsd), _HSyncData (hsd),
	  _lastCPUCycles (0),
	  _lastRasterLine (0),
	  _format (nullptr)
{
...
bool C64::VICII::simulate (MCHEmul::CPU* cpu)
{
	auto isBadRasterLine = [=]() -> bool
		{ return (_VICIIRegisters -> currentRasterLine () >= 0x0030 &&
				  _VICIIRegisters -> currentRasterLine () <= 0x007f &&
				  (_VICIIRegisters -> currentRasterLine () & _VICIIRegisters -> verticalScrollPosition ())); };

	if (_VICIIRegisters -> vicIItoGenerateIRQ ())
		cpu -> interrupt (F6500::IRQInterrupt::_ID) -> setActive (true);

	unsigned int cC = cpu -> clockCycles  ();
	for (size_t i = 0; i < (cC - _lastCPUCycles); i++)
	{
		if (_VICIIRegisters -> currentRasterLine () != _lastRasterLine)
		{
			if (isBadRasterLine ())
			{
				cpu -> addClockCycles (40);

				//  TODO: Read char memoy,...
			}

			_lastRasterLine = _VICIIRegisters -> currentRasterLine ();
		}

		if (_VICIIRegisters -> currentRasterLine () > _VSyncData.toBase0 ())
	}
