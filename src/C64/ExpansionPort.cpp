...
bool C64::ExpansionIOPort::simulate (MCHEmul::CPU* cpu)
{
	...
	if (expansionElement () != nullptr &&
		...
		notify (MCHEmul::Event (C64::ExpansionIOPort::_C64EXPANSIONPORTMEMLINESACTUALIZED, 
			(expansionElement () -> PIN_UP (C64::ExpansionIOPort::_GAME) ? 0 : 1) + // When it is active the value goes to 0...
			(expansionElement () -> PIN_UP (C64::ExpansionIOPort::_EXROM) ? 0 : 2))); // When active the value goes to 0...
