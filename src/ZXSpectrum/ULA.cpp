...
void ZXSPECTRUM::ULA::SoundFunction::debugULASoundCycle (MCHEmul::CPU* cpu, unsigned int i)
{
	...
	_deepDebugFile -> writeCompleteLine (className (), cpu -> clockCycles () - i, "Info Cyle", 
		MCHEmul::Attributes ({ 
			{ "Sound",	std::string (_ULARegisters -> buzzerSignal () ? "UP" : "DOWN") },
			{ "MICOut",	std::string (_ULARegisters -> MICOutputSignal () ? "UP" : "DOWN") },
			{ "EAROut",	std::string (_ULARegisters -> EAROutputSignal () ? "UP" : "DOWN") },
			{ "EARIn",	std::string (_ULARegisters -> EARInputSignal () ? "UP" : "DOWN") },
			{ "EARRead",	std::string (_ULARegisters -> EARReadSignal () ? "UP" : "DOWN") } }));
}
...
bool ZXSPECTRUM::ULA::simulate (MCHEmul::CPU* cpu)
{
			...
			i > 0; i--)
	{
		...
		// The /INT line is generated by the ULA timing, independently of its acceptance by the CPU.
		clockINTLine (cpu, cpu -> clockCycles () - (i >> 1));
			...
			_raster.hData ().currentPositionAtBase0 () == 0)
		{
			...
			// FLASH is generated from bit 4 of a five-bit ULA frame counter.
			clockFlashFrame ();
		}
	}

	// INT is level-sensitive: while /INT remains low, request it as soon as the Z80 can accept it.
	requestINTIfNeeded (cpu);
	...
	// If the MIC output signal changed, a signal to the datasette is send...
	// ...the datasette would manage that change as a modification in the information written if any...
	if (_ULARegisters -> MICOutputSignalChanged ()) // After ckecked it, the value returns to false...
		notify (MCHEmul::Event (MCHEmul::DatasetteIOPort::_WRITE,
			_ULARegisters -> MICOutputSignal () ? 1 : 0));
...
// ---
void ZXSPECTRUM::ULA::clockINTLine (MCHEmul::CPU* cpu, unsigned int cC)
{
	if (_INTLineActive && --_INTClocksRemaining == 0)
	{
		_INTLineActive = false;

		// Once an instruction boundary has been reached, the CPU owns the sampled request
		// and will remove it after the acknowledge cycle. Otherwise it is no longer valid.
		if (cpu -> state () != MCHEmul::CPU::_EXECUTINGINSTRUCTION ||
			cpu -> currentInstruction () != nullptr)
			removePendingINTRequest (cpu);
	}
	...
	{
		_INTLineActive = true;
		_INTClocksRemaining = _INTPULSEULACLOCKS;
		_INTStartCPUCycle = cC;
	}
}

// ---
void ZXSPECTRUM::ULA::requestINTIfNeeded (MCHEmul::CPU* cpu)
{
	FZ80::CZ80* z80 = static_cast <FZ80::CZ80*> (cpu);

	if (!_INTLineActive ||
		!cpu -> interrupt (FZ80::INTInterrupt::_ID) -> active () ||
		!z80 -> IFF1 () ||
		z80 -> instAfterEIToLaunchINT () != 0 ||
		INTRequestPending (cpu))
		return;

	cpu -> requestInterrupt
		(FZ80::INTInterrupt::_ID, _INTStartCPUCycle, this,
		 0 /** The reason is that the screen is complete. */);
}

// ---
bool ZXSPECTRUM::ULA::INTRequestPending (const MCHEmul::CPU* cpu) const
{
	for (const MCHEmul::CPUInterruptRequest& iR : cpu -> interruptsRequested ())
		if (iR.type () == FZ80::INTInterrupt::_ID && iR.from () == this)
			return (true);
...
// ---
void ZXSPECTRUM::ULA::removePendingINTRequest (MCHEmul::CPU* cpu) const
{
	const MCHEmul::CPUInterruptRequests requests = cpu -> interruptsRequested ();

	for (const MCHEmul::CPUInterruptRequest& iR : requests)
		if (iR.type () == FZ80::INTInterrupt::_ID && iR.from () == this)
			cpu -> removeInterruptRequest (iR);
}
...
void ZXSPECTRUM::ULA::processEvent (const MCHEmul::Event& evnt, MCHEmul::Notifier* n)
{
	...
	switch (evnt.id ())
	{
		...
		// Put the cassette signal in the independent EAR input for the simulation to read it!
		case MCHEmul::DatasetteIOPort::_READ:
			{
				_ULARegisters -> setEARInputSignal (evnt.value () == 1);
				// The cassette input is also audible in the simplified digital sound model.
...
bool ZXSPECTRUM::ULA::readGraphicInfoAndDrawVisibleZone (MCHEmul::CPU* cpu)
{
	...
	// FLASH exchanges INK and PAPER for 16 frames of every 32-frame cycle.
