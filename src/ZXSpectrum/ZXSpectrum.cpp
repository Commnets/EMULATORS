...
{ 
	...
	const MCHEmul::Instruction* inst = cpu () -> lastInstruction ();
	assert (inst != nullptr);
	...
	// I/O contention has already been applied by the instruction.
	// Determine whether the final address-bus access belonged to an I/O cycle.
	bool lastAccessWasIO = false;
	const FZ80::Instruction* dI =
		dynamic_cast <const FZ80::Instruction*> (inst);
	if (dI != nullptr)
		lastAccessWasIO = dI -> lastINOUTAccessWasIO ();
	else
	{
		const FZ80::InstructionUndefined* uI =
			dynamic_cast <const FZ80::InstructionUndefined*> (inst);
		assert (uI != nullptr);

		lastAccessWasIO = uI -> lastINOUTAccessWasIO ();
	}

	if (!lastAccessWasIO &&
		inst -> lastExecutionData ()._INOUTAddress.value () >= 0x4000 &&
