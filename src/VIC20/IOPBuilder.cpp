	...
	(int id, MCHEmul::Computer* c, const MCHEmul::Attributes& prms) const
{
	...
	if (id == MCHEmul::Typewriter::_ID)
		...
		{
			VIC20::CommodoreVIC20* v20 = dynamic_cast <VIC20::CommodoreVIC20*> (c);
			assert (v20 != nullptr); // it must be a vic20, but just in case...
			...
			if (VIC20::Disk1541Simulation::isDeviceNumberValid (dN)) // Only if it is valid...
				result = new VIC20::Disk1541Simulation 
					(v20 -> configuration (), c -> asciiToCodeConverter (), id, dN); // ...otherwise it will nullptr, and not created...
		}
