			...
			case Status::_READING:
				{
					...
					if (rNB)
					{
						...
						bool ov = false; // To know whether it was overflow in the reading or not...
						size_t pDC = _dataCounter;
						MCHEmul::UByte nVR = readFromData (ov);
						// If there was a change in the block managed...
						if (_dataCounter != pDC)
							_implementation -> whenReadingNewBlock (_data._data [_dataCounter]);
						// If there was overflow, the reading stops...
						if (ov) { _LOG ("Overflow while reading data from the file"); _status = Status::_STOPPED; }
