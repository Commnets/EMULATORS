	...
	(int id, MCHEmul::Computer* c, const MCHEmul::Attributes& prms) const
{
	...
	else if (id == ZX81::ThermalPrinterSimulation::_ID)
	{
		...
		std::tie (pF, mPE) = getDataPrinterFrom 
			(prms, std::make_tuple 
				(c -> asciiToCodeConverter (), // It is needed for some specific printer way...
		...
		if (mPE == nullptr) mPE = new MCHEmul::BasicMatrixPrinterEmulation 
			(c -> asciiToCodeConverter (), 32, pF); // Not usual, but just to avoid a crash later!
