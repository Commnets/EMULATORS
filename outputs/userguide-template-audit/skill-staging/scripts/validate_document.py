...
def validate(path: Path, allow_placeholders: bool) -> list[str]:
    ...
    with zipfile.ZipFile(path) as package:
        ...
        positions = []
        for heading in ("Índice", "Prólogo", "Anexos", "Ilustraciones", "Ecuaciones"):
            position = text.find(heading)
            if position < 0:
                errors.append(f"missing required block: {heading}")
        ...
        if "Página" not in footer_text or not re.search(r"\bPAGE\b", footer_fields) or not re.search(r"\bNUMPAGES\b", footer_fields):
            errors.append("footer does not contain Página plus PAGE/NUMPAGES fields")
