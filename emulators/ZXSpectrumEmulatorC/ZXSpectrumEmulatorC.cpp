...
int _tmain (int argc, _TCHAR *argv [])
{
	...
	MCHEmul::LogSystem::_LOGSYSTEM -> addLogChannel 
		(new MCHEmul::LogFileChannel (0, "./ZXSpectrumE.log", 10000 /** events max. */));
	MCHEmul::LogSystem::_LOGSYSTEM -> setDefaultChannelId (0);

	std::cout << "ZXSpectrumEmulator with Console" << std::endl;
	...
	MCHEmul::FormatterBuilder::instance ({ "./ZXSpectrum.fmt" });

	// Set up the emulator...
	ZXSPECTRUM::ZXSpectrumEmulator myEmulator (MCHEmul::CommandLineArguments (argc, argv), nullptr /** no commuications from outside. */);
	...
	MCHEmul::LocalConsole myConsole (&myEmulator, 
		new ZXSPECTRUM::CommandBuilder (new MCHEmul::StandardCommandBuilder),
